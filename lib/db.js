...
/**
 * Suma (o resta, con número negativo) MASTERCOINS al usuario.
 * Devuelve el nuevo saldo, o null si el usuario no existe.
 * No permite que el saldo quede negativo (se limita a 0).
 */
function modificarSaldo(m, cantidad) {
  const db = leerDB()
  const numero = numeroDeSender(m)
  if (!db[numero]) return null

  db[numero].mastercoins = Math.max(0, (db[numero].mastercoins || 0) + cantidad)
  guardarDB(db)
  return db[numero].mastercoins
}
