...
function listarSubbots() {
  ...
  // Subbots que están en memoria (ej. QR/código pendiente) pero aún no
  // terminaron de vincularse, por lo tanto no están en el registro guardado.
...
function eliminarSubbot(id) {
  ...
  // Si no existe ni en memoria ni en el registro guardado, no hay nada que borrar.
...
async function iniciarSubbot({ id, numeroCreador, chatCreador, metodo, numeroVincular, avisar }) {
  ...
  conn.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'close') {
      // Si este cierre fue provocado a propósito por eliminarSubbot(), no reconectar.
