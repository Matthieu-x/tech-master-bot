...
const { serializarMensaje, normalizarJid } = require('./serializar')
...
const CACHE_GRUPOS_TTL = 5 * 60 * 1000 // 5 minutos
const cacheGrupos = new Map() // chatId -> { tieneBotPrincipal, expira }

async function estaBotPrincipalEnGrupo(conn, chatId) {
  if (!global.mainBotJid) return false

  const cache = cacheGrupos.get(chatId)
  if (cache && cache.expira > Date.now()) return cache.tieneBotPrincipal

  try {
    const metadata = await conn.groupMetadata(chatId)
    const participantes = (metadata.participants || []).map((p) => normalizarJid(p.id))
    const tieneBotPrincipal = participantes.includes(global.mainBotJid)
    cacheGrupos.set(chatId, { tieneBotPrincipal, expira: Date.now() + CACHE_GRUPOS_TTL })
    return tieneBotPrincipal
  } catch (e) {
    return false
  }
}
...
async function iniciarSubbot({ id, numeroCreador, chatCreador, metodo, numeroVincular, avisar }) {
  ...
  conn.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    // Si el bot principal ya está en este mismo grupo, el subbot se queda callado
    // para no duplicar respuestas.
    if (m.isGroup && (await estaBotPrincipalEnGrupo(conn, m.chat))) return
